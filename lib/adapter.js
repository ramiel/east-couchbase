...
Adapter.prototype.executionByQueries = function(db, queries, callback) {
    ...
    async.each(queries,
        function(item, callback) {
            ...
            item = item.replace(/\$1/ig, bucket._name);
            var q = (N1qlQuery.fromString(item)).adhoc(false);
...
Adapter.prototype._getExecutedMigrations = function(callback) {
    var keyName = `${this.config.keyPrefix}${this.config.keySeparator}executed`;
...
Adapter.prototype.markExecuted = function(name, callback) {
    var objectKey = `${this.config.keyPrefix}${this.config.keySeparator}executed`;
...
Adapter.prototype.unmarkExecuted = function(name, callback) {
    ...
    this._getExecutedMigrations((err, result) => {
        ...
        var executedMigrations = (result.value || []).filter(m => m !== name);
        bucket.upsert(objectKey, executedMigrations, {cas: result.cas}, callback);
    });
};
